...
describe('Audio', () => {
  ...
  it('should poll for progress every 500ms if the file is played, and stop doing that when it is paused', () => {
    ...
    let intervalId;
    const setIntervalSpy = vi.spyOn(window, 'setInterval').mockImplementationOnce(() => {
  ...
  it('should show the correct progress', async () => {
    ...
    vi.spyOn(HTMLAudioElement.prototype, 'duration', 'get').mockImplementationOnce(() => 100);
    vi.spyOn(HTMLAudioElement.prototype, 'currentTime', 'get').mockImplementationOnce(() => 50);
