...
vi.mock('../MessageOptions', () => ({ MessageOptions: vi.fn(() => <div />) }));
vi.mock('../MessageText', () => ({ MessageText: vi.fn(() => <div />) }));
vi.mock('../../MML', () => ({ MML: vi.fn(() => <div />) }));
vi.mock('../../Avatar', () => ({ Avatar: vi.fn(() => <div />) }));
vi.mock('../../MessageInput', () => ({
  EditMessageForm: vi.fn(() => <div />),
  MessageInput: vi.fn(() => <div />),
}));
vi.mock('../../Modal', () => ({ Modal: vi.fn((props) => <div>{props.children}</div>) }));

const alice = generateUser();
const bob = generateUser({ image: 'bob-avatar.jpg', name: 'bob' });
const carol = generateUser();
const openThreadMock = vi.fn();
const tDateTimeParserMock = vi.fn((date) => Dayjs(date));
